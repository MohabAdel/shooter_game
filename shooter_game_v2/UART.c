...
void UART_Init(void){
  ...
  UART0_FBRD_R = 25;                     // FBRD = int(0.4028 * 64 + 0.5) = 25
                                        // 8 bit word length (no parity bits, one stop bit, FIFOs)
  UART0_LCRH_R = (UART_LCRH_WLEN_8|UART_LCRH_FEN);
  UART0_IFLS_R &= ~0x3F;                // clear TX and RX interrupt FIFO level fields
                                        // configure interrupt for TX FIFO <= 1/8 full
  //UART0_IFLS_R |=  UART_IFLS_RX7_8  ;      // 0x00000020  RX FIFO >= 7/8 full   
...
void UART1_Init(void){
  ...
  UART1_IBRD_R = 520;                    // IBRD = int(80,000,000 / (16 * 9600)) = int(520.833)
  UART1_FBRD_R = 54;                     // FBRD = int(0.833 * 64 + 0.5) = 54
                                        // 8 bit word length (no parity bits, one stop bit, FIFOs)
  ...
  GPIO_PORTC_AFSEL_R |= 0x30;           // enable alt funct on P	
  ...
  GPIO_PORTC_AMSEL_R = 0;               // disable analog functionality on PC
                                        // UART1=priority 2
  NVIC_PRI1_R = (NVIC_PRI1_R&0xFF0FFFFF)|0x00400000; // bits 20-23
